...
export function NewTodoForm() {
    ...
    const getTodos = async() => {
        ...
        try {
            ...
            const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/todo`)
            // Filter todos to include only those of the same day
                const today = new Date();
                const filteredTodos = response.data.result.filter(todo => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setPostLoading(true);
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/todo/create`, data);
